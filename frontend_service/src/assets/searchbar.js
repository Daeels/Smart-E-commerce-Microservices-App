...
const handleSuccess = function(stream) {
  ...
  mediaRecorder.addEventListener('stop', function() {
    ...
    reader.onloadend = function () {
      ...
      ajaxRequest.onreadystatechange = function(){
        if(ajaxRequest.readyState == 4){
          if (ajaxRequest.status == 200){
            ...
            localStorage.setItem("searched_item",ajaxRequest.responseText)
            window.location.replace("http://localhost:81/products/6/0")
          }
      ...
      ajaxRequest.open('POST', "http://127.0.0.1:5000/search-by-voice");
...
searchButton.addEventListener('click', function() {
  ...
  ajaxRequest.open('GET', "http://127.0.0.1:5000/search-by-text?text="+input);
  // ajaxRequest.setRequestHeader("Content-Type", "application/json");
  // data = JSON.stringify(data);
  ajaxRequest.send();
  // console.log(data);
